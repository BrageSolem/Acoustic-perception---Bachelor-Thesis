...
The average intereference time with the HAILO chip was found to be 9.2ms whereas with CPU only it was 235.6ms. (fig: \ref{fig:Interference_comp}) The Raspberry PI 5 was therefore found to be able to run the YOLO detection model at around 108 intereferences pr second, whereas for CPU only it was around 3.75. 
...
It was found that the CPU usage with the HAILO chip was at 29.7\% on average, where as without the CPU usage was found to be 92.5\% on average(fig: \ref{fig:cpu_usage_comparison}). Similarily the average CPU temperature with the HAILO chip was found to be 54.1 $C\degree$ where as with CPU only the average temperature rose to an average of 71 $C\degree$ (fig: \ref{fig:cpu_temp_comparison})
...
The RAM usage was found to be similar both with and without the HAILO chip (fig: \ref{fig:RAM Usage})
